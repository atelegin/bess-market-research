...
def test_project_capacity_detailed_from_ambient_matches_base_at_zero_c():
    """At C_rate→0 (no self-heating), the ambient entry point must reproduce
    the cell-internal-T entry point exactly — T_cell = T_amb + k·0² = T_amb."""
    from lib.models.degradation.detailed import (
        project_capacity_detailed,
        project_capacity_detailed_from_ambient,
    )
    preset = PRESETS["eve_lf280k"]
    # mean_crate=0 → no self-heating regardless of k; both paths should agree.
    duty = DutyCycle.from_mean(
        fec_per_year=0, mean_dod=0.80, mean_soc=0.55, mean_crate=0.0, mean_temp_C=30.0
    )
    soh_cell = project_capacity_detailed(duty, years=5.0, preset=preset, n_mc=1)
    soh_amb = project_capacity_detailed_from_ambient(
        duty, years=5.0, preset=preset, self_heating_k=2.0, n_mc=1
    )
    assert abs(soh_cell - soh_amb) < 1e-9, (soh_cell, soh_amb)


def test_project_capacity_detailed_from_ambient_adds_self_heating():
    """At mean_crate=2C with self_heating_k=2.0, ambient entry point should see
    T_cell = T_amb + 8 °C and produce visibly faster cycle-channel fade than
    treating the same T as cell-internal."""
    from lib.models.degradation.detailed import (
        project_capacity_detailed,
        project_capacity_detailed_from_ambient,
    )
    preset = PRESETS["eve_lf280k"]
    duty = DutyCycle.from_mean(
        fec_per_year=730, mean_dod=0.80, mean_soc=0.55, mean_crate=2.0, mean_temp_C=30.0
    )
    soh_cell = project_capacity_detailed(duty, years=10.0, preset=preset, n_mc=1)
    soh_amb = project_capacity_detailed_from_ambient(
        duty, years=10.0, preset=preset, self_heating_k=2.0, n_mc=1
    )
    assert soh_amb < soh_cell - 0.01, (soh_amb, soh_cell)
...
def test_from_mean_soc_anchor_invariance():
    ...
    """
    from lib.models.degradation.detailed import LFPGraphiteWangNaumann
